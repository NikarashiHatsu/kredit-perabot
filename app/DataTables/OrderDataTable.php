...
class OrderDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('action', 'dashboard.order.action')
            ...
            ->setRowClass(function($checkout) {
                if ($checkout->payments->count() > 0) {
                    return $checkout->payments->filter(function($payment) {
                        return $payment->is_read == false;
                    })->count() > 0 ? '!bg-yellow-100' : '';
                }

                if ($checkout->is_read == false) {
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::make('user.name')->title('Nama Pelanggan'),
